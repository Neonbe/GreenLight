...
## 🛑 What Error Messages Does GreenLight Fix?

macOS Gatekeeper displays different warnings depending on your OS version and the app's code signature. **GreenLight detects and fixes all of them automatically:**
...
If you see **any** of these messages after downloading an app, the application isn't actually broken or malicious — it's just flagged with `com.apple.quarantine`. **Instead of running terminal commands, GreenLight automatically sends you a one-click "Fix & Open" notification the instant Gatekeeper blocks an app.** No Terminal. No `xattr`. No manual work. Ever.

<details>
<summary><b>中文弹窗对照 | Chinese Error Messages</b></summary>
<br>

GreenLight 自动修复以下所有 macOS 系统弹窗：

- ❌ *"应用程序"已损坏，无法打开。您应该将它移到废纸篓。*
- ❌ *无法打开"应用程序"，因为无法验证开发者。macOS 无法验证此 App 不包含恶意软件。*
- ❌ *打不开"应用程序"，因为 Apple 无法检查其是否包含恶意软件。*
- ❌ *打不开"应用程序"，因为它来自身份不明的开发者。*
- ❌ *"应用程序"被阻止，因为它来自一个未识别的开发者。*

看到以上任何弹窗？应用并没有损坏或包含恶意软件——只是被 `com.apple.quarantine` 隔离标志拦截了。**安装 GreenLight 后，一切自动处理：自动检测 → 自动通知 → 一键修复。无需终端，无需命令，永远不用手动操作。**
